...
window.initRequestDataTable = function (allData, requestColumns, allAddData) {
            ...
            allData.forEach(function (device) {
                if (device.inventory?.inventoryToCustomerCareDeviceSendingStatus === 'Pending' ||
                    ...
                    // Unique rowKey (use stable fields to avoid duplicates)
                    let rowKeyParts = [
                        sn,
                        bivagName,
                        categoryName,
                        device.inventory?.inventoryToCustomerCareDeviceSendingStatus || "N/A",
                        device.inventory?.cooDeliveryAns || "Pending",
                        device.customerCare?.customerCareToDepartmentDeviceSendingStatus || "N/A",
                    ...
                    if (!currentRowMap.has(rowKey)) {
                        ...
                        htmlData += `
                            ...
                            <td onclick="window.trackDeviceRequestData(this.closest('tr'), this)"
                                ...
                                style="background-color: #007bff; color: #ffffff; text-align: center; padding: 10px; border-radius: 5px; cursor: pointer; font-weight: 500; transition: background-color 0.3s ease; font-size: 14px;"
                            ...
                            <td>
                                <div class="d-flex justify-content-center align-items-center action-button-container">
                                    ${device.inventory?.inventoryToCustomerCareDeviceSendingStatus === 'Pending' &&
                                      (device.inventory?.cooDeliveryAns === 'Accepted' || device.inventory?.cooDeliveryAns === 'Approved') ? `
                                        <button class="btn btn-sm text-white Accept"
                                            ...
                                            title="Accept Device">✔</button>
                                    ` : ""}
                                    ${device.inventory?.inventoryToCustomerCareDeviceSendingStatus === 'Accepted' &&
                                      device.customerCare?.customerCareToDepartmentDeviceSendingStatus !== 'Accepted' ? `
                                    ...
                                    <button class="btn btn-info btn-sm view-button chat"
                                        ...
                                        data-request-id="${device.id}"
                                        data-button-id="view"
                                        title="View Delivery device">&#128065;</button>
                                </div>
                            </td>
                        `;
